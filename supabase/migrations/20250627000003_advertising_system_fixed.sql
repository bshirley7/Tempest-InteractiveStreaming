/*
  # Advertising System Migration (Fixed)
  ...
  4. Fixed Issues
    ...
    - Conditional RLS policy creation
...
CREATE TABLE IF NOT EXISTS ad_analytics (
  ...
  content_id uuid, -- Will add FK constraint later if content table exists
  channel_id uuid, -- Will add FK constraint later if channels table exists
...
DO $$
BEGIN
    -- Add FK constraint for content table if it exists
    ...
    -- Add FK constraint for channels table if it exists
    ...
    -- Add FK constraints for user_profiles if it exists
...
-- Only create policies if user_profiles table exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_profiles') THEN
        -- Ad Videos Policies
    ...
    ELSE
        -- Fallback policies if user_profiles doesn't exist
...
-- Triggers for updated_at timestamps (only if the function exists)
DO $$
BEGIN
